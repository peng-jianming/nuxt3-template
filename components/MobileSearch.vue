...
<template>
  <div class="flex desktop:hidden">
    <div class="flex h-72 w-564 flex-1 items-center overflow-hidden rounded-l-10 bg-[#F4F4F4] leading-72">
      <!-- <svg class="box-content h-40 w-40 rounded-l-10 pr-10 pl-16" aria-hidden="true">
        <use xlink:href="#icon-icon_jiantoufuben" style="fill:currentColor; color:#FF5500" />
      </svg> -->
      <img class="h-24 w-24 ml-25 mr-15" src="/img/magnifier.png" alt="">
      <div class="flex h-30 flex-1 items-center">
        <input
          ref="inputRef" v-model.trim="inputValue" type="text" :placeholder="placeholder"
    ...
    <div
      class="h-72 w-127 bg-cover text-center text-32 leading-72 text-bgc" style="background-image: url(/img/search-btn.png);"
      @click="handleSearch"
    >
      搜索
    </div>
