...
<template>
  <div class="mt-60 desktop:mt-90 relative flex flex-col items-center">
    <div
    ...
    <div class="desktop:hidden">
      ...
      <div class=" text-primary text-25 text-center leading-25 cursor-pointer" @click="navigateTo('/trademark')">
        查看更多 >>
      </div>
    </div>
    <!-- PC端 -->
    <div class="hidden desktop:flex w-594 mt-15 z-10 relative m-auto justify-between">
      <div
        v-for="item in list" :key="item.id" class="w-188 h-188 rounded-16  overflow-hidden shadow-input cursor-pointer"
        @click="navigateTo(`/trademark/detail/${item.id}`)"
      >
      ...
      <img
        class="w-41 h-27 absolute top-1/2 -translate-y-1/2 -right-55 cursor-pointer" src="/img/index/arrow-right.png"
