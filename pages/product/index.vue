...
<template>
  ...
  <div class="desktop:hidden">
    ...
    <div class="mobile-category">
      <van-tabs v-model:active="activeSecond" line-width="0" class="px-30" swipe-threshold="4" :ellipsis="false"
        style="--van-tabs-line-height: 0.666rem" title-inactive-color="#999">
        <van-tab v-for="item in menu.child" :key="item.id" :name="item.title">
          <template #title>
            <div class="text-31 px-15 font-black h-50 leading-50 bg-bgc font-OPPOSans-B"
              :class="[{ 'under-bg-green-four': activeSecond === item.title }]" @click="getList(item.type, item.id)">
              {{
                item.title
              }}
            </div>
          ...
          <van-tabs v-if="item.child.length" v-model:active="activeThrid" shrink line-width="0"
            class="mt-15 pt-10 boder-info" :ellipsis="false" style="--van-tabs-nav-background: #FFFDF7">
            <van-tab v-for="(_item, index) in item.child" :key="index" :name="_item.title">
              <template #title>
                <div title-active-color="#fff"
                  ...
                  @click="getList(_item.type, _item.id)">
                  {{
                    _item.title
                  }}
                </div>
        ...
        <div v-if="listParams.list.length">
          <van-list v-model:loading="listParams.loading" :finished="listParams.finished" @load="listParams.onload">
            <div class="py-30 min-h-500 relative flex flex-wrap justify-between">
              <div v-for="item in listParams.list" :key="item.id" class=" shrink-0 w-330 mb-30">
  ...
  <div class="hidden desktop:block">
    <div class="pt-88 ml-68 flex -mb-100 relative z-30">
      ...
      <img class="absolute -left-70 top-340 w-191 h-89" src="/img/download/left-point.png" alt="">
      <!-- 菜单 -->
        ...
        <van-collapse v-model="activeAll" :border="false">
          <van-collapse-item name="all">
            ...
            <van-collapse v-model="activeSecond" :border="false" accordion class="pl-5 pt-13 bg-transparent">
              <van-collapse-item v-for="item in menu.child" :key="item.id" :title="item.title" :name="item.title"
                :border="false">
                <template #title>
                  <div class="font-OPPOSans-B text-11 text-black  inline-block px-3 pb-3"
                    :class="[{ 'under-bg-green-four': activeSecond === item.title }]"
                    @click="getList(item.type, item.id)">
                    {{ item.title }}
                  </div>
                </template>
                <div v-if="item.child.length" class="pl-14 pt-6 bg-transparent">
        ...
        <div v-if="listParams.list.length">
          <van-list v-model:loading="listParams.loading" :finished="listParams.finished" @load="listParams.onload">
            <div class="flex flex-wrap">
              <div v-for="(item, index) in listParams.list" :key="index" class="w-130 mb-20 cursor-pointer"
                ...
                <div class="px-14">
                  ...
                  <div class="text-6 text-info line-clamp-2">
                    {{ item.product_desc }}
                  </div>
                </div>
              </div>
